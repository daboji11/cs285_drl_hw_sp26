...
def iter_minibatches(
    ...
    if shuffle:
        indices = torch.randperm(n=N, generator=generator)
    else:
    ...
    for start in range(0, N, minibatch_size):
        ...
        task_names = (
            [batch.task_names[i] for i in idx.tolist()]
            if batch.task_names is not None
            else None
        )

        completion_texts = (
            [batch.completion_texts[i] for i in idx.tolist()]

## HW1 Imitation Learning （done）

### MSE Policy
- Action chunking with MLP
- Input: state
- Output: flattened action chunk
- Loss: squared L2 loss over the whole chunk
wandb: Run summary:
wandb: eval/mean_reward 0.69675
wandb:             step 75600
wandb:       train/loss 0.26424

### Flow Matching Policy
- Input: state, noisy action chunk, flow timestep tau
- Training target: action_chunk - noise
- Interpolation: A_tau = tau * A + (1 - tau) * A_0
- Inference: Euler integration from Gaussian noise
wandb: eval/mean_reward 0.83862
wandb:             step 75600
wandb:       train/loss 3.0252

### Notes
- Increasing MLP capacity significantly improved the flow policy.
- Flow matching achieved better rollout performance than the MSE policy.

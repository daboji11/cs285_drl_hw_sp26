...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t in range(length):
            return_sum += rewards[t] * self.gamma ** t
        ans = [return_sum] * length
        return ans
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t in range(length):
            ...
            for tp in range(t, length):
                return_sum += rewards[tp] * self.gamma ** (tp - t)
            ans[t] = return_sum
        return ans
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        q_values = batch_size * [None]
        if not self.use_reward_to_go:
            # Case 1: in trajectory-based PG, we ignore the timestep and instead use the discounted return for the entire
            # trajectory at each point.
            # In other words: Q(s_t, a_t) = sum_{t'=0}^T gamma^t' r_{t'}
            for curr_trajectory in range(batch_size):
                q_values[curr_trajectory] = self._discounted_return(rewards=rewards[curr_trajectory])
        else:
            # Case 2: in reward-to-go PG, we only use the rewards after timestep t to estimate the Q-value for (s_t, a_t).
            # In other words: Q(s_t, a_t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
            for curr_trajectory in range(batch_size):
                q_values[curr_trajectory] = self._discounted_reward_to_go(rewards=rewards[curr_trajectory])
        return q_values
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    #   error   = r(s_t, a_t) + gamma * V(s_t+1) - V(s_t)
                    if terminals[i] == 0:
                        error_i = rewards[i] + self.gamma * values[i+1] - values[i]
                        advantages[i] = error_i + self.gae_lambda * self.gamma * advantages[i+1]
                    else:
                        error_i = rewards[i] - values[i]
                        advantages[i] = error_i

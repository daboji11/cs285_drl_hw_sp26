...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = nn.Parameter(torch.tensor(np.log(temperature), dtype=float))
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                next_qs = next_qs + next_action_entropy * self.temperature_tensor()
                # Hint: next_qs = ...
                # ENDTODO
    ...
    def temperature_tensor(self):
        if self.auto_tune_temperature:
            ...
            return torch.tensor(self.temperature, device=ptu.device)
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        loss -= entropy * self.temperature_tensor()
        # ENDTODO
    ...
    def update(
        ...
        for _ in range(self.num_critic_updates):
            info = self.update_critic(obs=observations, 
                                      action=actions,
                                      reward=rewards,
